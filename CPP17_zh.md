...
- [C++17](#c17)
  - [概述](#概述)
  - [C++17 语言特性](#c17-语言特性)
    - [Class Template Argument Deduction(类模板参数推导)](#class-template-argument-deduction类模板参数推导)
    - [Declaring Non-Type Template Parameters with Auto(使用 auto 声明非类型模板参数)](#declaring-non-type-template-parameters-with-auto使用-auto-声明非类型模板参数)
    - [Folding Expression(折叠表达式)](#folding-expression折叠表达式)
    - [New Rules for Auto Deduction from Braced Init List(auto 从花括号初始化列表推导的新规则)](#new-rules-for-auto-deduction-from-braced-init-listauto-从花括号初始化列表推导的新规则)
    - [Constexpr Lambda(constexpr lambda)](#constexpr-lambdaconstexpr-lambda)
    - [Lambda Capture this by Value(Lambda 按值捕获 this)](#lambda-capture-this-by-valuelambda-按值捕获-this)
    - [Inline Variable(内联变量)](#inline-variable内联变量)
    - [Nested Namespace(嵌套命名空间)](#nested-namespace嵌套命名空间)
    - [Structured Binding(结构化绑定)](#structured-binding结构化绑定)
    - [Selection Statements with Initializer(带初始化器的选择语句)](#selection-statements-with-initializer带初始化器的选择语句)
    - [Constexpr If(constexpr if)](#constexpr-ifconstexpr-if)
    - [UTF-8 Character Literal(UTF-8 字符字面量)](#utf-8-character-literalutf-8-字符字面量)
    - [Direct List Initialization of Enums(枚举的直接列表初始化)](#direct-list-initialization-of-enums枚举的直接列表初始化)
    - [\[\[fallthrough\]\], \[\[nodiscard\]\], \[\[maybe\_unused\]\] 属性](#fallthrough-nodiscard-maybe_unused-属性)
  ...
  - [C++17 标准库特性](#c17-标准库特性)
    ...
    - [std::byte](#stdbyte)
    - [Splicing for Maps and Sets(map 和 set 的拼接)](#splicing-for-maps-and-setsmap-和-set-的拼接)
    - [Parallel Algorithm(并行算法)](#parallel-algorithm并行算法)
    - [std::sample](#stdsample)
    - [std::clamp](#stdclamp)
    - [std::reduce](#stdreduce)
    - [Prefix Sum Algorithm(前缀和算法)](#prefix-sum-algorithm前缀和算法)
    - [GCD and LCM(gcd 和 lcm)](#gcd-and-lcmgcd-和-lcm)
    - [std::not\_fn](#stdnot_fn)
    - [String Conversion to/from Numbers(字符串与数字的转换)](#string-conversion-tofrom-numbers字符串与数字的转换)
    - [Rounding Functions for Chrono Durations and Timepoints(chrono 持续时间和时间点的舍入函数)](#rounding-functions-for-chrono-durations-and-timepointschrono-持续时间和时间点的舍入函数)
  - [致谢](#致谢)
  - [作者](#作者)
  - [内容贡献者](#内容贡献者)
  - [许可证](#许可证)
