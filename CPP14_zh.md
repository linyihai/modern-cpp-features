...
- [C++14](#c14)
  - [概述](#概述)
  - [C++14 语言特性](#c14-语言特性)
    - [Binary Literal(二进制字面量)](#binary-literal二进制字面量)
    - [Generic Lambda Expression(泛型 Lambda 表达式)](#generic-lambda-expression泛型-lambda-表达式)
    - [Lambda Capture Initializer(Lambda 捕获初始化器)](#lambda-capture-initializerlambda-捕获初始化器)
    - [Return Type Deduction(返回类型推导)](#return-type-deduction返回类型推导)
    - [decltype(auto)](#decltypeauto)
    - [Relaxing Constraints on Constexpr Functions(放松 constexpr 函数的约束)](#relaxing-constraints-on-constexpr-functions放松-constexpr-函数的约束)
    - [Variable Template(变量模板)](#variable-template变量模板)
    - [\[\[deprecated\]\] Attribute(\[\[deprecated\]\] 属性)](#deprecated-attributedeprecated-属性)
  - [C++14 标准库特性](#c14-标准库特性)
    - [标准库类型的用户自定义字面量](#标准库类型的用户自定义字面量)
    - [Compile-Time Integer Sequence(编译时整数序列)](#compile-time-integer-sequence编译时整数序列)
